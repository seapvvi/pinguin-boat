...
export const data = new SlashCommandBuilder()
  .setName('youtube')
  .setDescription('Gerer les notifications YouTube Live')
  .addSubcommand((sub) =>
    sub.setName('add').setDescription('Ajouter une chaine YouTube a surveiller')
      .addStringOption((opt) =>
        opt.setName('channel').setDescription('Nom de la chaine YouTube').setRequired(true)
      )
  ...
  .addSubcommand((sub) =>
    sub.setName('remove').setDescription('Supprimer une notification YouTube')
      .addStringOption((opt) =>
        opt.setName('channel').setDescription('Nom de la chaine YouTube').setRequired(true).setAutocomplete(true)
      )
  )
  .addSubcommand((sub) =>
    sub.setName('list').setDescription('Lister les notifications YouTube configurees')
  );
...
const platform: StreamPlatform = 'YOUTUBE';
...
async function handleAdd(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  await interaction.editReply({
    embeds: [successEmbed('YouTube ajoute', channelName + ' sera surveille. Les notifications seront envoyees dans <#' + channel.id + '>.')],
  });
}
...
async function handleRemove(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  await interaction.editReply({
    embeds: [successEmbed('YouTube supprime', channelName + ' ne sera plus surveille.')],
  });
}
...
async function handleList(interaction: ChatInputCommandInteraction): Promise<void> {
  ...
  if (notifications.length === 0) {
    await interaction.editReply({ embeds: [errorEmbed('Aucune notification', 'Aucune chaine YouTube configuree.')] });
    return;
  }
