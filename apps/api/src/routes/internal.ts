...
function requireInternalSecret(
  ...
  const secretHash = hash(INTERNAL_SECRET);
  const headerHash = hash(headerSecret);

  if (secretHash.length !== headerHash.length || !crypto.timingSafeEqual(secretHash, headerHash)) {
    reply.status(403).send(error('Forbidden'));
    return;
  }
}
